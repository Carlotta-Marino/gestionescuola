...
namespace Gestione
{
    internal class Program
    {
        ...
        static void AggiungiVoto()
        {
            ...
            // Calcola la media aggiornata per la materia specifica
            AggiornaMediaValutazioni(idMateria);
        }

        static void AggiornaMediaValutazioni(int idMateria)
        {
            List<Valutazione> valutazioniMateria = valutazioni.FindAll(v => v.IDMateria == idMateria);

            float sommaVoti = 0;
            int numeroValutazioni = 0;

            foreach (var valutazione in valutazioniMateria)
            {
            ...
            if (numeroValutazioni > 0)
            {
                ...
                // Aggiorna la media solo per le valutazioni della materia specifica
                foreach (var valutazione in valutazioniMateria)
                {
                    valutazione.Media = media;
                }

                Console.WriteLine("Media valutazioni aggiornata con successo.");
            }
        }
        ...
        static void ModificaVoto()
        {
            ...
            if (votoDaModificare != null)
            {
                ...
                // Calcola la media aggiornata per la materia specifica
                AggiornaMediaValutazioni(votoDaModificare.IDMateria);
            }
            else
            {
        ...
        static void CancellaVoto()
        {
            ...
            if (votoDaCancellare != null)
            {
                ...
                // Aggiorna la media solo per la materia specifica associata al voto cancellato
                AggiornaMediaValutazioni(idMateria);
            }
            else
            {
